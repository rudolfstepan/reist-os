...
static int syscall_device_control(uint32_t command, const void *user_request,
    ...
    if (command == DEVICE_DOMAIN_CONTROL_DMA_RELOCATE_AND_SEAL) {
        if (user_result != NULL) return -22;
        device_domain_dma_relocation_request_t request;
        if (copy_from_user(&request, user_request, sizeof(request)) != 0)
            return -14;
        return device_domain_dma_relocate_and_seal(
            process->pid, process->generation, &request);
    }
    if (command == DEVICE_DOMAIN_CONTROL_DMA_VM_PAGE_MODE) {
        if (user_result != NULL) return -22;
        device_domain_dma_vm_page_mode_request_t request;
        if (copy_from_user(&request, user_request, sizeof(request)) != 0)
            return -14;
        return device_domain_dma_vm_page_mode(
            process->pid, process->generation, &request);
    }
    if (command == DEVICE_DOMAIN_CONTROL_GR_FIRMWARE_UPLOAD) {
        if (user_result != NULL) return -22;
        device_domain_gr_firmware_request_t request;
        if (copy_from_user(&request, user_request, sizeof(request)) != 0)
            return -14;
    ...
    if (command == DEVICE_DOMAIN_CONTROL_GR_EXECUTE) {
        if (!device_output_accessible(
                directory, user_result,
                sizeof(device_domain_gr_execution_result_t))) return -14;
        device_domain_gr_execution_request_t request;
        device_domain_gr_execution_result_t result;
        if (copy_from_user(&request, user_request, sizeof(request)) != 0)
            return -14;
        int status = device_domain_gr_execute(
            process->pid, process->generation, &request, &result);
        if (status != 0) return status;
        return copy_to_user_space(directory, (uint32_t)(uintptr_t)user_result,
                                  &result, sizeof(result)) == 0 ? 0 : -14;
    }
    if (command == DEVICE_DOMAIN_CONTROL_GR_CONTEXT_MEMORY) {
        if (!device_output_accessible(
                directory, user_result,
                sizeof(device_domain_gr_context_memory_result_t))) return -14;
        device_domain_gr_context_memory_request_t request;
        device_domain_gr_context_memory_result_t result;
        if (copy_from_user(&request, user_request, sizeof(request)) != 0)
            return -14;
        int status = device_domain_gr_context_memory(
            process->pid, process->generation, &request, &result);
        if (status != 0) return status;
        return copy_to_user_space(directory, (uint32_t)(uintptr_t)user_result,
                                  &result, sizeof(result)) == 0 ? 0 : -14;
    }
    if (command == DEVICE_DOMAIN_CONTROL_GR_GOLDEN_CONTEXT) {
        if (!device_output_accessible(directory, user_result,
                sizeof(device_domain_gr_golden_context_result_t))) return -14;
        device_domain_gr_golden_context_request_t request;
        device_domain_gr_golden_context_result_t result;
        if (copy_from_user(&request, user_request, sizeof(request)) != 0)
            return -14;
        int status = device_domain_gr_golden_context(
            process->pid, process->generation, &request, &result);
        if (status != 0) return status;
        return copy_to_user_space(directory, (uint32_t)(uintptr_t)user_result,
                                  &result, sizeof(result)) == 0 ? 0 : -14;

...
static int prepare(void *output,const void *elf,size_t length,unsigned wide) {
    ...
    if(!a || !b || length<64 || length>(wide?524288U:65536U) || a>UINTPTR_MAX-bytes ||
    ...
    unsigned char flags[64]={0};unsigned executable=0;
    ...
    put(r,wide?0x0000327647504e52ULL:0x0000317647504e52ULL,8);
    put(r+8,wide?2:1,4);put(r+12,bytes,4);put(r+16,entry,8);

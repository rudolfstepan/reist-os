...
static void mutate(void *p) {
    ...
    for(unsigned i=0;i<REIST_X64_PREPARED_V2_BYTES;i++) b[i]=0x5a;
}
#elif PROGRAM_ID==2
...
static volatile struct memory_witness witness __attribute__((section(".data.memory_witness")))={0x594d454d45444957ULL,0,0,0,0,0};
...
int main(int argc,char **argv) {
    ...
    for(unsigned round=0;round<2;round++) {
        unsigned mode=round?0:MEMORY_CASE;
        if(mode==6)mode=0;
        ...
        REQUIRE(reist_x64_image_prepare_v2(record,import_blob,sizeof(import_blob))==0,203);
        if(round) {
            /* Distinct valid immutable bytes force a fresh image on slot reuse. */
            unsigned matches=0;unsigned char *b=record;
            for(unsigned p=16;p+20<64;p++) {
                unsigned off=96+p*4096;
...
#if MEMORY_CASE==6
        if(!round){REQUIRE(child==-12,204);child=reist_x64_task_import_wide(record,&profile,32,&startup);}
#endif

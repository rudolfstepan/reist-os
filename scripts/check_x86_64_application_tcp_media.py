"""Independent signed TCP profile consumer; accepted BL profile stays strict."""
...
NAMES=('boot.prg','cat.prg','ls.prg','probe.prg','data.txt','netdrv.prg','netstack.prg','udp.prg','nc.prg')
selected=clone(legacy,[
 ("PROFILE='research-native-shell-two-media-v1'","PROFILE='research-native-application-tcp-two-media-v1'"),
 ("'data':'primary-master-read-only-ext2-1k'","'data':'primary-master-read-only-application-tcp-ext2-1k'"),
 ("'file-program.prg':1280","'file-program.prg':274432, 'cat.prg':274432, 'ls.prg':274432, 'probe.prg':274432, 'data.txt':16384, 'netdrv.prg':274432, 'netstack.prg':274432, 'udp.prg':274432, 'nc.prg':274432"),
 ("'system.ext2':131072","'system.ext2':1048576"),('64<=len(app)<=1280','1536<len(app)<=274432'),
 ("directory/'shell-media.json'","directory/'application-tcp-media.json'"),
 ("'file-program.prg',*(f'program{n}.prg' for n in range(4)))",
  "'file-program.prg',*(f'program{n}.prg' for n in range(4)),'cat.prg','ls.prg','probe.prg','data.txt','netdrv.prg','netstack.prg','udp.prg','nc.prg')"),
 ("data_volume(bounded(attempt/'system.ext2',131072),values['file-program.prg'])",
  "data_volume(bounded(attempt/'system.ext2',1048576),values)")],'reist_application_tcp_consumer')
...
def input_binding(values):
    need(set(values)==set(accepted.PINS)|{'netdrv.prg','netstack.prg','udp.prg','nc.prg'},'exact TCP profile files')
...
exec(compile(source,'<application-tcp-ext2-consumer>','exec'),namespace)
...
if __name__=='__main__':
    ...
    print('APPLICATION_TCP_MEDIA_VERIFY_OK',verify(a.directory,a.index,a.openssl))

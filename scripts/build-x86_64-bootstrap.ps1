...
if ($NativeVideoMode) { $NativeVgaConsole = [switch]$true }
if ($NativeVgaConsole) {
...
if ($NativeShell) { $NativeConsole = [switch]$true }
if ($NativeConsole) {
...
if ($NativeServiceCPU) {
    ...
    $NativeTaskPool = [switch]$true
}
if ($NativePoolPIO) {
    if ((-not $NativeLiveFile -and ($NativeFilesystem -or $NativeFileLaunch)) -or $MemoryCase -ne 0 -or $PIOCase -ne 0 -or
        $StartupCase -ne 0 -or $FamilyCase -ne 0 -or $BlockProfileCase -ne 0 -or $FilesystemCase -ne 0 -or
        $FilesystemLayout -ne 2 -or $FileLaunchCase -ne 0 -or $ProgramCase -ne 0) {
        throw 'NativePoolPIO excludes filesystem, file-launch and fault selectors.'
    }
    $NativeTaskPool = [switch]$true
    $NativeBlockProfile = [switch]$true
}
if ($NativeTaskPool) {
    if ((-not $NativePoolPIO -and ($NativePIO -or $NativeBlock -or $NativeBlockProfile)) -or (-not $NativeLiveFile -and ($NativeFilesystem -or $NativeFileLaunch)) -or
...
if (-not $NativeFilesystem -and ($FilesystemCase -ne 0 -or $FilesystemLayout -ne 2)) { throw 'Filesystem options require NativeFilesystem.' }
if ($NativeBlockProfile) {
    if ($MemoryCase -ne 0 -or $PIOCase -ne 0 -or $StartupCase -ne 0) { throw 'NativeBlockProfile excludes other memory/PIO/startup fixtures.' }
    $NativeWide = [switch]$true
    $NativeBlock = [switch]$true
}
if ($BlockProfileCase -ne 0 -and -not $NativeBlockProfile) { throw 'BlockProfileCase requires NativeBlockProfile.' }
if ($NativeWide) {
    if (-not $NativeBlockProfile -and ($NativePIO -or $NativeBlock -or $PIOCase -ne 0 -or $StartupCase -ne 0)) { throw 'NativeWide excludes PIO/block/startup fault fixtures.' }
...
if ($NativeStartup) {
    ...
    $NativeLifecycle = [switch]$true
}
if ($FamilyCase -ne 0 -and -not $NativeLifecycle) { throw 'FamilyCase requires NativeLifecycle.' }
if ($NativeLifecycle) {
    if ($ProgramCase -ne 0) { throw 'NativeLifecycle excludes ProgramCase.' }
    $NativePrograms = [switch]$true
}
if ($NativePrograms) {
...
if ($NativeHeap -and (-not $NativeRAM -or -not $NativeProcesses)) {
    throw 'NativeHeap requires NativeRAM and NativeProcesses.'
}
...
try {
    ...
    & $Make $nativeBuildTarget `
        ...
        "X86_64_NATIVE_VIDEO_MODE=$([int]$NativeVideoMode.IsPresent)" `

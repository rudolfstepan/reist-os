"""Build the ordinary nslookup.prg with its explicit native object adapter."""
...
def build_tool(directory,cc,nasm,ld):
    ...
    def run(command):
        ...
        with (directory/'application-dns-build.log').open('ab') as log:log.write(p.stdout+p.stderr)
    ...
    common=[*cc,'-target','x86_64-freestanding-none','-std=c11','-Oz','-Wall','-Wextra','-Werror',
        ...
        '-ffunction-sections','-fdata-sections','-DREIST_NATIVE_APP_TCP=1','-DREIST_NATIVE_APP_DNS=1','-Iuserspace/sdk/include','-fstack-usage']
    start=directory/'dns-start.o';run([*nasm,'-f','elf64','arch/x86_64/user/boot_start.asm','-o',start])
    objects=[]
    for n,source in enumerate(['userspace/programs/nslookup.c','userspace/sdk/lib/x86_64/application_tcp.c','userspace/sdk/lib/x86_64/application_udp.c',
            'userspace/sdk/lib/x86_64/application_dns.c','userspace/sdk/reist_dns.c',
            'userspace/sdk/lib/x86_64/application_tcp_platform.c','userspace/sdk/lib/x86_64/network_session.c']):
        obj=directory/f'dns-{n}.o';run([*common,'-c',source,'-o',obj]);objects.append(obj)
    elf=directory/'nslookup.prg'
    run([*ld,'-m','elf_x86_64','-nostdlib','--build-id=none','--fatal-warnings','--no-undefined',
         ...
         '-Map='+str(directory/'dns.map'),'-o',elf,start,*objects])
    raw=elf.read_bytes();prepare(raw,[],True)
    (directory/'root/nslookup.prg').write_bytes(raw)
    return elf

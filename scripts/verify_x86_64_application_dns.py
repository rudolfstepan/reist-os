"""Frozen native application DNS gates in a separate evidence namespace."""
...
evidence=clone(previous,[
    ("EVIDENCE=ROOT/'build/codex-agent/r83bj-network-dma'","EVIDENCE=ROOT/'build/codex-agent/r83bp-application-dns'"),
    ("BASE=EVIDENCE/'candidate02'","BASE=EVIDENCE/'candidate04'"),
    ("HEAD='c6533954'","HEAD='387ae858'"),
    ("PACKAGE='R8.3bj-network-dma'","PACKAGE='R8.3bp-application-dns'"),
    ("BUILD='03'","BUILD='03'")], 'reist_application_dns_evidence')
def tools():
...
def source_projection():
    v=evidence;macro='REIST_NATIVE_APP_DNS'
    def project(s):
    ...
    def replace(s,a,b=''):
        v.need(s.count(a)==1,'one explicit DNS addition '+a[:60]);return s.replace(a,b)
    for n in ('userspace/sdk/lib/x86_64/shell_session.c','userspace/programs/native_netstack.c',
    ...
    a=s.index('# Explicit DNS application profile;');b=s.index('X86_64_NATIVE_INPUT ?=',a)
    s=s[:a]+s[b:]
    s=replace(s,'X86_64_SESSION_ARG += $(if $(filter 1,$(X86_64_NATIVE_APP_DNS)),--app-dns,)\n')
    a=s.index('.PHONY: x86_64-application-dns-media');b=s.index('.PHONY: x86_64-application-tcp-media',a)
    s=s[:a]+s[b:];v.need(s==v.original(n),'exact disabled Make rules')
    class Disabled(ast.NodeTransformer):
        def visit_If(self,n):
            if any(isinstance(x,ast.Name) and x.id=='app_dns' for x in ast.walk(n.test)):
                v.need(not n.orelse,'no hidden default branch');return None
            return self.generic_visit(n)
        def visit_FunctionDef(self,n):
            if n.name in ('build','build_roles') and n.args.args[-1].arg=='app_dns':
                n.args.args.pop();n.args.defaults.pop()
            return self.generic_visit(n)
        def visit_Expr(self,n):
            if isinstance(n.value,ast.Call) and any(isinstance(a,ast.Constant) and a.value=='--app-dns' for a in n.value.args):return None
            return self.generic_visit(n)
        def visit_Call(self,n):
            n=self.generic_visit(n)
            if n.args and (isinstance(n.args[-1],ast.Name) and n.args[-1].id=='app_dns' or
                          isinstance(n.args[-1],ast.Attribute) and n.args[-1].attr=='app_dns'):n.args.pop()
            return n
    ...
    # The shared parser/cache and FIFO transport corrections are deliberate
    # fixes for legacy callers too. Their ABI is retained and actual old/new
    # behavior is covered by the frozen host gate.
...
def defaults():
    ...
    print('APPLICATION_DNS_DEFAULTS_OK')
def freeze():
...
def regression_inputs():
    v=evidence;paths=[]
    for name in ('diagnostic04','diagnostic05','diagnostic06','diagnostic07',
                 'diagnostic08','diagnostic09','diagnostic10','diagnostic11','candidate03/guests/tcp-fragmented','candidate02/guests/tcp-fragmented'):
        folder=v.EVIDENCE/name;v.need(folder.is_dir(),'retained diagnostic '+name)
        paths.extend(p for p in folder.iterdir() if p.is_file() and
            (p.suffix in ('.bin','.json') or p.name in ('guest.log','frame-trace.log')))
    paths.append(v.EVIDENCE/'build05/x86_64/reist-x86_64-bootstrap.elf')
    paths.append(v.EVIDENCE/'development-host11.json')
    paths.append(v.EVIDENCE/'scope-stop.json')
    paths.extend(v.EVIDENCE/n for n in ('development-host20.json','development-host22.json',
        'candidate01/acceptance-seal.json','candidate01/post-gate-review-rejection.json',
        'candidate02/stopped.json','candidate02/build/x86_64/reist-x86_64-bootstrap.elf',
        'build06/x86_64/reist-x86_64-bootstrap.elf',
        'build07/x86_64/reist-x86_64-bootstrap.elf',
        'build08/x86_64/reist-x86_64-bootstrap.elf',
        'build09/x86_64/reist-x86_64-bootstrap.elf','cpu-scope-stop.json',
        'development-host27.json','development-host28.json','development-host29.json',
        'development-host30.json','development-host31.json',
        'candidate03/stopped.json','candidate03/build/x86_64/reist-x86_64-bootstrap.elf',
        'build10/x86_64/reist-x86_64-bootstrap.elf'))
...
def image():
    ...
    from run_qemu_x86_64_application_dns import stack_review
...
def package():
    ...
    run('reference-build',[f['tools']['pwsh']['path'],'-NoProfile','-File','scripts/build-x86_64-bootstrap.ps1',
        '-NativeAppDNS','-OutputDirectory',folder.relative_to(v.ROOT).as_posix()],300)
    run('reference-media',[sys.executable,'scripts/build_x86_64_application_dns_media.py','--input-directory',
    ...
    v.save(v.BASE/'package.json',dict(passed=True,stack=stack,image=v.link(folder/'x86_64/reist-x86_64-bootstrap.elf'),
        index=v.link(media/'application-dns-media.json'),artifacts={p.relative_to(v.ROOT).as_posix():v.digest(p)
...
def runtime():
    ...
    for name in guest.CASES:
        ...
        v.save(v.BASE/(name+'.json'),row);print('APPLICATION_DNS_CASE_OK',name,flush=True)
...
def review():
    ...
    print('APPLICATION_DNS_REVIEW_OK')
def gates():

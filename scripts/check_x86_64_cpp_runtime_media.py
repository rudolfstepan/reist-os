"""Independent native C/C++ consumer; all accepted CLI inputs remain pinned."""
...
NAMES=('boot.prg','cat.prg','ls.prg','probe.prg','data.txt','cpptest.prg')
selected=clone(legacy,[
    ("PROFILE='research-native-shell-two-media-v1'","PROFILE='research-native-cpp-runtime-two-media-v1'"),
    ("'data':'primary-master-read-only-ext2-1k'","'data':'primary-master-read-only-cpp-runtime-ext2-1k'"),
    ("'file-program.prg':1280","'file-program.prg':274432,'cat.prg':274432,'ls.prg':274432,'probe.prg':274432,'data.txt':16384,'cpptest.prg':274432"),
    ("'system.ext2':131072","'system.ext2':1048576"),('64<=len(app)<=1280','1536<len(app)<=274432'),
    ("directory/'shell-media.json'","directory/'cpp-runtime-media.json'"),
    ("'file-program.prg',*(f'program{n}.prg' for n in range(4)))",
     "'file-program.prg',*(f'program{n}.prg' for n in range(4)),'cat.prg','ls.prg','probe.prg','data.txt','cpptest.prg')"),
    ("data_volume(bounded(attempt/'system.ext2',131072),values['file-program.prg'])",
     "data_volume(bounded(attempt/'system.ext2',1048576),values)")], 'reist_cpp_runtime_consumer')
...
def input_binding(values):
    need(type(values) is dict and set(values)==set(accepted.PINS)|{'cpptest.prg'},'exact native C++ inputs')
    accepted.input_binding({n:values[n] for n in accepted.PINS})
    validate_cpp_object(values['cpptest.prg'],architecture='x86_64')
    selected.programs.prepare(values['cpptest.prg'],[],True)
...
source=once(source,'(32,1024,12,1,0,0,1024,1024,32,1,11,2)','(32,1024,15,1,0,0,1024,1024,32,1,11,2)')
source=once(source,"b'\\xff\\xff\\x0f\\x00'","b'\\xff\\xff\\x01\\x00'")
source=once(source,'(free,free,12,1)','(free,free,15,1)')
source=once(source,"'free_inodes':12","'free_inodes':15")
namespace={**vars(geometry),'NAMES':NAMES}
exec(compile(source,'<cpp-runtime-ext2-consumer>','exec'),namespace)
...
if __name__=='__main__':
    ...
    print('CPP_RUNTIME_MEDIA_VERIFY_OK',verify(a.directory,a.index,a.openssl))

"""Signed eight-file immutable native application UDP profile."""
from pathlib import Path
import argparse,inspect,sys
import build_x86_64_shell_media as legacy
import build_x86_64_graphical_media as geometry
import check_x86_64_application_udp_media as check
from run_qemu_x86_64_runtime_clock import once
NAMES=check.NAMES
source=inspect.getsource(geometry.image)
for old,new in [('(16,12)','(16,13)'),('(1<<20)-1','(1<<19)-1'),('u16(2048+14,12)','u16(2048+14,13)')]:
    source=once(source,old,new)
namespace={**vars(geometry),'NAMES':NAMES}
exec(compile(source,'<application-udp-ext2-producer>','exec'),namespace)
image=namespace['image']
selected=check.clone(legacy,[
 ('scripts/check_x86_64_shell_media.py','scripts/check_x86_64_application_udp_media.py'),
 ("output/'shell-media.json'","output/'application-udp-media.json'"),
 ("for name in ('file-program.prg',*(f'program{n}.prg' for n in range(4))):",
  "for name in ('file-program.prg',*(f'program{n}.prg' for n in range(4)),'cat.prg','ls.prg','probe.prg','netdrv.prg','netstack.prg','udp.prg'):"),
 ...
 ("'system.ext2 is the separate primary ATA master, read-only, containing /boot.prg.\\n'",
  "'system.ext2 is read-only; udp uses explicit application grants and separate Ring3 services.\\n'")],
 'reist_application_udp_producer')

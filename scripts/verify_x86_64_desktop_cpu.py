"""Frozen qualification for selected plan-v6/CREATE-v9; all failures retain their receipts."""
from pathlib import Path
import argparse, hashlib, json, os, re, subprocess, sys, time, tomllib
import verify_x86_64_display as common
import run_qemu_x86_64_desktop_cpu as guest

ROOT=common.ROOT
BASE=ROOT/'build/codex-agent/r83cg-desktop-cpu'
...
def package():
    ...
    need(len(active)==1 and active[0]['id']==queue['active_id']=='R8.3cg-desktop-cpu','one package')
    return active[0]
...
def freeze():
    ...
    need(baseline['code']==0 and baseline['head'].startswith(HEAD),'preimplementation baseline')
    need(artifacts(BASE/'baseline01')==read(BASE/'baseline-artifacts.json'),'preserved complete baseline')
...
def build(label,enabled):
    ...
    run(['pwsh','-NoProfile','-File','scripts/build-x86_64-bootstrap.ps1',
         '-NativeDesktopCPU' if enabled else '-NativeLargePeriodic',
...
def defaults():
    ...
    command=[common.build_tools()['zig']['path'],'cc','-DREIST_NATIVE_LARGE_IMAGE=1','-DREIST_NATIVE_LARGE_PERIODIC=1',
...
def runtime():
    ...
    for case,oom in ((0,None),(4,None),(5,None),(6,26),(3,None),(1,None),(7,None)):
        ...
        print('DESKTOP_CPU_GUEST_OK',case,round(elapsed,3),flush=True)
    need(time.monotonic()-started<=2520,'aggregate guest deadline')
...
def review():
    ...
    image=media_binding();rows=read(GATES/'runtime.json');need(len(rows)==7 and [r['case'] for r in rows]==[0,4,5,6,3,1,7],'seven complete cases')
...
def gates():
    ...
    for i,(command,limit) in enumerate(zip(commands,(180,600,600,3000,1200)),1):
        binding();run(command.split(),GATES/f'gate-{i}.log',limit);binding()
        print('DESKTOP_CPU_GATE_OK',i,flush=True)

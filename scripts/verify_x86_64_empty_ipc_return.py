...
import run_qemu_x86_64_empty_ipc_return as guest

ROOT=common.ROOT
BASE=ROOT/'build/codex-agent/r83ci-empty-ipc'
BASELINE=ROOT/'build/codex-agent/r83ch-query-return/baseline01'
ARCHIVE=ROOT/'build/codex-agent/r83cf-desktop-startup/after-routing78/candidate.zip'
ARCHIVE_SHA='d9d27ab6823b83a70c0c55762140bafc1e59d08063da2576bfc760a22110dada'
GATES=BASE/'qualification03'
CASES=((9,None),(10,None),(11,None),(0,None),(4,None),(5,None),(6,26),(3,None),(1,None),(7,None),(8,None))
need=common.need
save=common.save
read=common.read
...
def package():
    ...
    need(len(active)==1 and queue['active_id']==active[0]['id']=='R8.3ci-empty-ipc-return','one active CI package')
    return active[0]

def scope():
    need(set(common.common.changed())<=set(package()['allowed_files']),'frozen CI source scope')
    subprocess.run(['git','diff','--check'],cwd=ROOT,check=True,capture_output=True,timeout=30)

def freeze():
    scope()
    need(common.common.git('rev-parse','--short=8','HEAD')=='5ff06c69','CI contract baseline')
    need(not GATES.exists(),'fresh qualification')
    need(common.artifacts(BASELINE)==read(BASELINE.parent/'baseline-artifacts.json'),'preserved baseline56')
    save(GATES/'frozen.json',dict(head=common.common.git('rev-parse','HEAD'),
        ...
        contract=digest(ROOT/'docs/architecture/NATIVE_EMPTY_IPC_RETURN_CONTRACT.md'),
        font=digest(ROOT/'assets/fonts/reist-vga.psf'),
        baseline=read(BASELINE.parent/'baseline-artifacts.json'),archive=ARCHIVE_SHA,
...
def binding():
    ...
    need(f['contract']==digest(ROOT/'docs/architecture/NATIVE_EMPTY_IPC_RETURN_CONTRACT.md'),'approved frozen contract')
    need(f['font']==digest(ROOT/'assets/fonts/reist-vga.psf'),'immutable pixel oracle font')
    need(digest(ARCHIVE)==f['archive'] and common.artifacts(BASELINE)==f['baseline'],'immutable baseline/archive')
    for name,sha in f['history'].items():need(digest(BASE/name)==sha,'retained development history')
    return f

def defaults():
    common.BASE=BASELINE.parent;common.GATES=GATES
    common.defaults()
...
def runtime():
    ...
    need(time.monotonic()-started<=3960,'aggregate eleven-guest deadline')
...
def gui():
    ...
    baseline=ROOT/'build/codex-agent/r83cf-desktop-startup/diagnostic78'
...
def review():
    ...
    need([(r['case'],r['oom']) for r in rows]==list(CASES),'all eleven frozen cases')
    ...
    for row in rows:
        ...
        need(('-icount' in command)==(row['case'] in (8,9,10,11)),'only query cases use instruction time')
    ...
    d=read(GATES/'defaults.json');old=read(BASELINE.parent/'baseline-artifacts.json');key='x86_64/programs/program0.o'
...
def gates():
    ...
    for i,(command,limit) in enumerate(zip(commands,(180,600,600,4200,1800,300)),1):
...
def diagnostic_gui(folder,existing=None):
    ...
    with module.overlay(files,folder/'cf-overlay'):
        ...
        with module.overlay(integration,folder/'integration-overlay'):
            common.run(['pwsh','-NoProfile','-File','scripts/build-x86_64-bootstrap.ps1','-NativeFullDesktop',
                '-OutputDirectory',str((folder/'build').relative_to(ROOT))],folder/'build.log',300)
            common.run([sys.executable,'scripts/build_x86_64_full_desktop_media.py','--input-directory',
                str(folder/'build/x86_64'),'--output-directory',str(folder/'media')],folder/'media.log',180)
            import run_qemu_x86_64_full_desktop as runner
            need(runner.SNAPSHOTS[0]==('tasks','scheduler_tasks',8192),'known task snapshot extent')
            runner.SNAPSHOTS=(('tasks','scheduler_tasks',32768),)+runner.SNAPSHOTS[1:]
            result=module.exercise(folder/'media',folder/'guest',capture=True)

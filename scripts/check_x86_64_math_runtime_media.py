"""Independent native mathematics consumer; all accepted CLI inputs remain pinned."""
from pathlib import Path
import argparse,inspect
import check_x86_64_shell_media as legacy
import check_x86_64_cpp_runtime_media as accepted
...
NAMES=(*accepted.NAMES,'mathtest.prg')
selected=clone(legacy,[
    ("PROFILE='research-native-shell-two-media-v1'","PROFILE='research-native-math-runtime-two-media-v1'"),
    ("'data':'primary-master-read-only-ext2-1k'","'data':'primary-master-read-only-math-runtime-ext2-1k'"),
    ("'file-program.prg':1280","'file-program.prg':274432,'cat.prg':274432,'ls.prg':274432,'probe.prg':274432,'data.txt':16384,'cpptest.prg':274432,'mathtest.prg':274432"),
    ("'system.ext2':131072","'system.ext2':1048576"),('64<=len(app)<=1280','1536<len(app)<=274432'),
    ("directory/'shell-media.json'","directory/'math-runtime-media.json'"),
    ("'file-program.prg',*(f'program{n}.prg' for n in range(4)))",
     "'file-program.prg',*(f'program{n}.prg' for n in range(4)),'cat.prg','ls.prg','probe.prg','data.txt','cpptest.prg','mathtest.prg')"),
    ("data_volume(bounded(attempt/'system.ext2',131072),values['file-program.prg'])",
     "data_volume(bounded(attempt/'system.ext2',1048576),values)")], 'reist_math_runtime_consumer')
...
def input_binding(values):
    need(type(values) is dict and set(values)==set(accepted.accepted.PINS)|{'cpptest.prg','mathtest.prg'},'exact native math inputs')
    accepted.input_binding({n:v for n,v in values.items() if n!='mathtest.prg'})
    ...
    validate_cpp_object(values['mathtest.prg'],architecture='x86_64')
    selected.programs.prepare(values['mathtest.prg'],[],True)
...
source=once(source,'(32,1024,12,1,0,0,1024,1024,32,1,11,2)','(32,1024,14,1,0,0,1024,1024,32,1,11,2)')
source=once(source,"b'\\xff\\xff\\x0f\\x00'","b'\\xff\\xff\\x03\\x00'")
source=once(source,'(free,free,12,1)','(free,free,14,1)')
source=once(source,"'free_inodes':12","'free_inodes':14")
namespace={**vars(geometry),'NAMES':NAMES}
exec(compile(source,'<math-runtime-ext2-consumer>','exec'),namespace)

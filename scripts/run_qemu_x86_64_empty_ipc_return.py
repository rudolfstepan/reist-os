...
def query_transport(case):
    if case not in (8,9,10,11):return cpu.transport
...
def query_before():
    ...
    if IPC_CASE in (10,11) and not query_count and number!=54:return
    assert number in (22,42,54)
    if number==54:
        assert q(S['syscall_rdx'])==0 and value==0xfffffffffffffff5
        assert q(S['scheduler_cpu_budgets']+8)==64
        assert q(S['scheduler_cpu_windows'])==100
    else:assert value==(t[1] if number==22 else q(S['scheduler_last_tick'])*10)
    query_count+=1
...
def replay(folder,serial,trace,count):
    ...
    samples={};operations={};caps={};finishes=0;name=None;reads=[];registers={}
        ...
        if 'begin' in row:name=row['begin'];reads=[];registers={}
            ...
            if name in ('query_before','query_cap') and any(at==s['query_state64'] for at,_ in reads):
                ...
                if name=='query_before':
                    ...
                    if config['query_case'] in (10,11) and task[1] not in samples and number!=54:
                        name=None;continue
                    need(number in (22,42,54),'query/empty IPC syscall identity')
                    if number==54:
                        need(struct.unpack('<Q',one(s['syscall_rdx'],8))[0]==0,'empty poll timeout0')
                        need(struct.unpack('<Q',one(s['scheduler_cpu_budgets']+8,8))[0]==64,'authoritative64 budget')
                        need(struct.unpack('<Q',one(s['scheduler_cpu_windows'],8))[0]==100,'authoritative100 tick period')
                        expected=2**64-11
                    else:expected=task[1] if number==22 else struct.unpack('<Q',one(s['scheduler_last_tick'],8))[0]*10
                    need(registers['rax']==expected,'query actual return value')
                    samples.setdefault(task[1],[]).append(state[2])
                    operations.setdefault(task[1],[]).append(number)
    ...
    if config['query_case'] in (8,9,10,11):
        need(len(samples)==2 and all(len(values)==8 for values in samples.values()),'two bounded query samples')
        if config['query_case']!=11:
            need(caps.keys()==samples.keys(),'actual eighth-query boundary for both roots')
    if config['query_case']==10:
        need(all(set(v)=={22,42,54} for v in operations.values()),'mixed clock PID empty polls')
    if config['query_case']==11:
        need(all(set(v)=={54} for v in operations.values()),'empty IPC flood')
    return dict(lifecycle=proof,queries=samples,operations=operations,caps=caps,cleared=finishes)

def capture(image,folder,case=0,oom=None):
    need(case in range(12),'query fixture case')
    ...
    equivalent={8:0,9:7,10:0,11:7}.get(case,case)
    ...
    if case==1:
        # Keep the entry trap disarmed through the fully observed release.
        # The free/return hooks still verify every frame and complete scrub.
        replacements += [
            ("Hook('scheduler_release_task_frames64',release_begin)",
             "release_hook=Hook('scheduler_release_task_frames64',release_begin)"),
            ("ReleaseEnd(release['ret']);free_hook.enabled=True",
             "ReleaseEnd(release['ret']);free_hook.enabled=True;release_hook.enabled=False"),
            ("            release=None\n",

"""Build the ordinary curl.prg with its explicit native object adapter."""
...
def build_tool(directory,cc,nasm,ld):
    ...
    def run(command):
        ...
        with (directory/'application-http-build.log').open('ab') as log:log.write(p.stdout+p.stderr)
    ...
    cc=[x for x in cc if not str(x).startswith(('-DREIST_NATIVE_NETWORK_SESSION','-DREIST_NATIVE_APP_DNS'))]
    common=[*cc,'-target','x86_64-freestanding-none','-std=c11','-Oz','-Wall','-Wextra','-Werror',
        ...
        '-ffunction-sections','-fdata-sections','-DREIST_NATIVE_APP_TCP=1','-DREIST_NATIVE_APP_HTTP=1','-Iuserspace/sdk/include','-Iuserspace/tls/include','-fstack-usage']
    start=directory/'http-start.o';run([*nasm,'-f','elf64','arch/x86_64/user/boot_start.asm','-o',start])
    objects=[]
    for n,source in enumerate(['userspace/programs/curl.c','userspace/programs/curl_http.c',
            'userspace/sdk/lib/x86_64/application_tcp.c','userspace/sdk/lib/x86_64/application_http.c',
            'userspace/sdk/lib/x86_64/application_tcp_platform.c','userspace/sdk/lib/x86_64/network_session.c']):
        obj=directory/f'http-{n}.o';run([*common,'-c',source,'-o',obj]);objects.append(obj)
    elf=directory/'curl.prg'
    run([*ld,'-m','elf_x86_64','-nostdlib','--build-id=none','--fatal-warnings','--no-undefined',
         ...
         '-Map='+str(directory/'http.map'),'-o',elf,start,*objects])
    raw=elf.read_bytes();prepare(raw,[],True)
    (directory/'root/curl.prg').write_bytes(raw)
    return elf

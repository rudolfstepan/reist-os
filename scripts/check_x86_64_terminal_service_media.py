...
selected=clone(legacy,[
 ("PROFILE='research-native-shell-two-media-v1'","PROFILE='research-native-terminal-service-two-media-v1'"),
 ...
 ("directory/'shell-media.json'","directory/'terminal-service-media.json'"),
 ...
 ("data_volume(bounded(attempt/'system.ext2',131072),values['file-program.prg'])",
  "data_volume(bounded(attempt/'system.ext2',1048576),values)")],'reist_terminal_service_consumer')
...
def input_binding(values):
    ...
    need('native_terminal_service_state' in symbols,'actual protected service authority')
    need(b'TERMINAL_SERVICE_OK\n' in values['file-program.prg'],'actual service consumer')

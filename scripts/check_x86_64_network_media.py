"""Independent native network package consumer; old media profiles stay strict."""
...
NAMES=('boot.prg','cat.prg','ls.prg','probe.prg','data.txt','netdrv.prg','netstack.prg')
selected=clone(legacy,[
 ("PROFILE='research-native-shell-two-media-v1'","PROFILE='research-native-network-session-two-media-v1'"),
 ("'data':'primary-master-read-only-ext2-1k'","'data':'primary-master-read-only-network-ext2-1k'"),
 ("'file-program.prg':1280","'file-program.prg':274432, 'cat.prg':274432, 'ls.prg':274432, 'probe.prg':274432, 'data.txt':16384, 'netdrv.prg':274432, 'netstack.prg':274432"),
 ("'system.ext2':131072","'system.ext2':1048576"),
 ('64<=len(app)<=1280','1536<len(app)<=274432'),
 ("directory/'shell-media.json'","directory/'network-media.json'"),
 ("'file-program.prg',*(f'program{n}.prg' for n in range(4)))",
  "'file-program.prg',*(f'program{n}.prg' for n in range(4)),'cat.prg','ls.prg','probe.prg','data.txt','netdrv.prg','netstack.prg')"),
 ("data_volume(bounded(attempt/'system.ext2',131072),values['file-program.prg'])",
  "data_volume(bounded(attempt/'system.ext2',1048576),values)")],'reist_network_consumer')
...
def input_binding(values):
    need(set(values)==set(accepted.PINS)|{'netdrv.prg','netstack.prg'},'exact network build inputs')
    ...
    need(values['program0.prg'].count(hashes)==1,'signed root binds both distinct network roles')
...
def medium_files(values):
    return {n:values['file-program.prg' if n=='boot.prg' else n] for n in NAMES}

# Same standard EXT2 consumer, explicitly adapted only for seven file inodes.
# Neither its old module globals nor its old exported verifier are mutated.
source=inspect.getsource(geometry.verify_volume)
for old,new in [('(32,1024,12,1,','(32,1024,14,1,'),
                ("b'\\xff\\xff\\x0f\\x00'","b'\\xff\\xff\\x03\\x00'"),
                ("'exact20 allocated inodes'","'exact18 allocated inodes'"),
                ('(free,free,12,1)','(free,free,14,1)'),("'free_inodes':12","'free_inodes':14")]:
    source=once(source,old,new)
namespace={**vars(geometry),'NAMES':NAMES}
exec(compile(source,'<network-ext2-consumer>','exec'),namespace)
...
if __name__=='__main__':
    ...
    print('NETWORK_MEDIA_VERIFY_OK',verify(a.directory,a.index,a.openssl))

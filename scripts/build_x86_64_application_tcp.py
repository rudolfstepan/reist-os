"""Build the ordinary tcp.prg with its explicit native object adapter."""
...
def build_tool(directory,cc,nasm,ld):
    ...
    def run(command):
        ...
        with (directory/'application-tcp-build.log').open('ab') as log:log.write(p.stdout+p.stderr)
    ...
    common=[*cc,'-target','x86_64-freestanding-none','-std=c11','-Oz','-Wall','-Wextra','-Werror',
        ...
        '-ffunction-sections','-fdata-sections','-DREIST_NATIVE_APP_TCP=1','-Iuserspace/sdk/include','-fstack-usage']
    start=directory/'tcp-start.o';run([*nasm,'-f','elf64','arch/x86_64/user/boot_start.asm','-o',start])
    objects=[]
    for n,source in enumerate(['userspace/programs/nc.c','userspace/sdk/lib/x86_64/application_tcp.c',
            'userspace/sdk/lib/x86_64/application_tcp_platform.c','userspace/sdk/lib/x86_64/network_session.c']):
        obj=directory/f'tcp-{n}.o';run([*common,'-c',source,'-o',obj]);objects.append(obj)
    elf=directory/'nc.prg'
    run([*ld,'-m','elf_x86_64','-nostdlib','--build-id=none','--fatal-warnings','--no-undefined',
         ...
         '-Map='+str(directory/'tcp.map'),'-o',elf,start,*objects])
    raw=elf.read_bytes();prepare(raw,[],True)
    (directory/'root/nc.prg').write_bytes(raw)
    return elf

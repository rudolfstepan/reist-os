...
CASES=(('healthy1024','',16,4096),('healthy800','',2,4096),('8g','',16,8192),
       ...
       ('service-crash','c',16,4096),('service-cpu','d',16,4096),('service-hang','e',16,4096))
import run_qemu_x86_64_display as display
from run_x86_64_display import data_fixture
need=check.need

def validate_cli_service_probe(row,starts,live,ordinary):
    """Only the canonical, denied startup grant extends the old CLI oracle."""
    request=bytes.fromhex(row['before'])
    if len(request)!=24 or struct.unpack_from('<I',request,8)[0]!=6:
        return ordinary(row,starts,live)
    gen=row['gen'];peer=gen+1
    need(row['op']==127 and row['slot']==0 and not row['profile_denied'] and
         row['size']==24 and len(row['args'])==6 and not any(row['args'][1:]),'service CLI root ABI')
    need(0<gen<0x7fffffff and request==struct.pack('<6I',1,24,6,0,peer,peer) and
         row['after']==request.hex(),'service CLI exact immutable foreign grant')
    need(gen in live and peer in live and starts.get(gen)==dict(slot=0,gen=gen,parent=0) and
         starts.get(peer)==dict(slot=1,gen=peer,parent=0),'service CLI actual independent live root')
    lease=struct.pack('<3Q',gen<<32,0,0).hex()
    need(row['result']==-13 and row['terminal_before']==row['terminal']==lease,
         'service CLI denied grant without lease publication')

def cli_namespace(label):
    selected=display.cli_namespace(label);module=selected.ay
    ordinary=module.validate_terminal
    module.validate_terminal=lambda row,starts,live:validate_cli_service_probe(row,starts,live,ordinary)
    return selected
...
def state(folder,step,mode,final=False,symbols=None):
    ...
    need(all(service[n]^service[n+4]==2**64-1 for n in range(4)),'protected terminal service inverse')
    need(service[3]==1 and not any((path/'terminal-proposal.bin').read_bytes()),'service fenced and proposal scrubbed')
    lease=struct.unpack('<3Q',(path/'terminal-lease.bin').read_bytes())
    need(lease[1]==0 and lease[2]==0,'service lease retired without console fence')
    if final:need(service[0]==service[1]==0 and service[2]>0 and lease[0]==0,'root service cleanup with epoch retained')
    elif service[0]:need(service[0]&0xffffffff==0 and service[1]&0xffffffff==4 and service[0]>>32>0 and service[1]>>32>0,'exact retired service generations')
    proof['terminal_service']=dict(root=service[0],owner=service[1],epoch=service[2],acquisitions=struct.unpack('<Q',(path/'terminal-acquisitions.bin').read_bytes())[0])
...
def live_profile(path,event):
    ...
    service=struct.unpack('<8Q',(path/'terminal-service.bin').read_bytes());lease=struct.unpack('<3Q',(path/'terminal-lease.bin').read_bytes())
    need(all(service[n]^service[n+4]==2**64-1 for n in range(4)),'live protected service state')
    need(service[0]==root and service[1]==event[5] and service[2]>0 and service[3]==0,'exact live designated service')
    need(lease==(root,event[5],0),'actual exclusive service lease')
    return dict(root=root,driver=event[4],consumer=event[5],epoch=event[6],service_epoch=service[2])
...
def run_case(image,package,folder,spec,*,diagnostic=False):
    ...
    try:
        values=__import__('build_x86_64_terminal_service_media').inputs(Path(image).parent)
        ...
        def snapshot(step,final=False):
            ...
            for name,symbol,size in [('boot','native_display_boot',64),('state','native_display_state',128),
                ...
                ('terminal-service','native_terminal_service_state',64),('terminal-lease','native_terminal_state',24),
                ('terminal-proposal','native_terminal_service_proposal',72),('terminal-acquisitions','native_terminal_service_acquisitions',8),
        ...
        first='boot.prg'+(' '+mode if mode in ('u','q','h','b','s','m','c','d','e') else '')
        ...
        for number,command in enumerate(commands):
            ...
            healthy=is_input and mode!='n' and not(number==0 and mode in ('u','q','h','b','m','p','f','c','d','e'))
            ...
            if inject:
                ...
                for name,symbol,size in (('tasks','scheduler_tasks',8192),('profiles','family_profiles',256),('family','family_records',512),('terminal-service','native_terminal_service_state',64),('terminal-lease','native_terminal_state',24)):
...
def review_case(image,package,folder,spec):
    ...
    for number,(segment,cmd) in enumerate(zip(row['segments'],commands)):
        ...
        healthy=cmd.startswith('boot.prg') and mode!='n' and not(number==0 and mode in ('u','q','h','b','m','p','f','c','d','e'))
            ...
            if number==0 and mode=='p':
                ...
                if mode=='n':
                    ...
                    if healthy:
                        ...
                        need(len(records)==1 and b'INPUT_READY\n' in text,'service fault only after healthy acquisition')
                        need(child[2:4]=={'c':(134,3),'d':(256,3),'e':(0,3)}[mode],'actual declared terminal service containment')
                        need(worker[2:4] in ((0,4),(0,3)),'independent input driver normal exit or bounded cancellation')
                        if mode=='d':need(child[4]==32,'actual service CPU32 exhaustion')
                    elif mode in ('u','q','h'):
    ...
    services=1 if mode=='p' else 3
    need(last['terminal_service']['epoch']==services and last['terminal_service']['acquisitions']==services,'all actual service grants/acquisitions and retained epochs')
    need(serial.count(b'TERMINAL_SERVICE_OK\n')==services,'actual ordinary service consumer self-tests')

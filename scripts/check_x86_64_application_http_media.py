"""Independent signed HTTP profile consumer; accepted BL profile stays strict."""
...
NAMES=('boot.prg','cat.prg','ls.prg','probe.prg','data.txt','netdrv.prg','netstack.prg','udp.prg','nc.prg','nslookup.prg','curl.prg')
selected=clone(legacy,[
 ("PROFILE='research-native-shell-two-media-v1'","PROFILE='research-native-application-http-two-media-v1'"),
 ("'data':'primary-master-read-only-ext2-1k'","'data':'primary-master-read-only-application-http-ext2-1k'"),
 ("'file-program.prg':1280","'file-program.prg':274432, 'cat.prg':274432, 'ls.prg':274432, 'probe.prg':274432, 'data.txt':16384, 'netdrv.prg':274432, 'netstack.prg':274432, 'udp.prg':274432, 'nc.prg':274432, 'nslookup.prg':274432, 'curl.prg':274432"),
 ("'system.ext2':131072","'system.ext2':1048576"),('64<=len(app)<=1280','1536<len(app)<=274432'),
 ("directory/'shell-media.json'","directory/'application-http-media.json'"),
 ("'file-program.prg',*(f'program{n}.prg' for n in range(4)))",
  "'file-program.prg',*(f'program{n}.prg' for n in range(4)),'cat.prg','ls.prg','probe.prg','data.txt','netdrv.prg','netstack.prg','udp.prg','nc.prg','nslookup.prg','curl.prg')"),
 ("data_volume(bounded(attempt/'system.ext2',131072),values['file-program.prg'])",
  "data_volume(bounded(attempt/'system.ext2',1048576),values)")],'reist_application_http_consumer')
...
def input_binding(values):
    need(set(values)==set(accepted.PINS)|{'netdrv.prg','netstack.prg','udp.prg','nc.prg','nslookup.prg','curl.prg'},'exact HTTP profile files')
    ...
    selected.programs.prepare(values['nc.prg'],[],True)
    selected.programs.prepare(values['nslookup.prg'],[],True)
    selected.programs.prepare(values['curl.prg'],[],True)
...
source=once(source,'first=32','first=33')
source=once(source,'(32,1024,12,1,0,0,1024,1024,32,1,11,2)','(32,1024,10,1,0,0,1024,1024,32,1,11,2)')
source=once(source,"b'\\xff\\xff\\x0f\\x00'","b'\\xff\\xff\\x3f\\x00'")
source=once(source,'(free,free,12,1)','(free,free,10,1)')
source=once(source,"'free_inodes':12","'free_inodes':10")
namespace={**vars(geometry),'NAMES':NAMES}
exec(compile(source,'<application-http-ext2-consumer>','exec'),namespace)
...
if __name__=='__main__':
    ...
    print('APPLICATION_HTTP_MEDIA_VERIFY_OK',verify(a.directory,a.index,a.openssl))

"""Frozen native application UDP gates in a separate evidence namespace."""
...
evidence=clone(previous,[
    ("EVIDENCE=ROOT/'build/codex-agent/r83bj-network-dma'","EVIDENCE=ROOT/'build/codex-agent/r83bm-application-udp'"),
    ("BASE=EVIDENCE/'candidate02'","BASE=EVIDENCE/'candidate04'"),
    ("HEAD='c6533954'","HEAD='35bfdcf2'"),
    ("PACKAGE='R8.3bj-network-dma'","PACKAGE='R8.3bm-application-udp'"),
    ("BUILD='03'","BUILD='06'")], 'reist_application_udp_evidence')
def tools():
...
def source_projection():
    ...
    v=evidence
    def replace(s,a,b=''):
        v.need(s.count(a)==1,'one explicit profile addition '+a[:60]);return s.replace(a,b)
    for n in ('userspace/sdk/lib/x86_64/shell_session.c','userspace/programs/udp.c',
        ...
        s=disabled((v.ROOT/n).read_text(encoding='utf-8'), 'REIST_NATIVE_APP_NETWORK')
        if n.endswith('network_session.h'):s=replace(s,'#define REIST_NET_LAST REIST_NET_HELLO\n')
        if n.endswith('network_session.c'):s=s.replace('REIST_NET_LAST','REIST_NET_HELLO')
        v.need(s.rstrip()==v.original(n).rstrip(),'exact disabled production '+n)
    n='scripts/build-x86_64-bootstrap.ps1';s=(v.ROOT/n).read_text(encoding='utf-8')
    for part in ('    [switch]$NativeAppNetwork,\n',
    ...
    begin=s.index('# Explicit application UDP authority;');end=s.index('X86_64_NATIVE_INPUT ?=',begin)
    s=s[:begin]+s[end:]
    s=replace(s,'X86_64_SESSION_ARG += $(if $(filter 1,$(X86_64_NATIVE_APP_NETWORK)),--app-network,)\n')
    begin=s.index('.PHONY: x86_64-application-udp-media');end=s.index('.PHONY: x86_64-network-media',begin)
    s=s[:begin]+s[end:];v.need(s==v.original(n),'exact disabled Make rules')
    class Disabled(ast.NodeTransformer):
        def visit_If(self,n):
            if any(isinstance(x,ast.Name) and x.id=='app_network' for x in ast.walk(n.test)):
                v.need(not n.orelse,'no hidden default branch');return None
            return self.generic_visit(n)
        def visit_FunctionDef(self,n):
            if n.name in ('build','build_roles') and n.args.args[-1].arg=='app_network':
                n.args.args.pop();n.args.defaults.pop()
            return self.generic_visit(n)
        def visit_Expr(self,n):
            if isinstance(n.value,ast.Call) and any(isinstance(a,ast.Constant) and a.value=='--app-network' for a in n.value.args):return None
            return self.generic_visit(n)
        def visit_Call(self,n):
            n=self.generic_visit(n)
            if n.args and (isinstance(n.args[-1],ast.Name) and n.args[-1].id=='app_network' or
                          isinstance(n.args[-1],ast.Attribute) and n.args[-1].attr=='app_network'):n.args.pop()
            return n
...
def defaults():
    ...
    print('APPLICATION_UDP_DEFAULTS_OK')
def freeze():
    ...
    v.save(v.BASE/'frozen.json',dict(head=v.git('rev-parse','HEAD'),package=p,changed=paths,sources=v.sources(),
        tools=tools(),regression_inputs=regression_inputs(),commands=commands,limits=[600,600,600,3900,600]))

def regression_inputs():
    v=evidence;folder=v.EVIDENCE/'diagnostic08'
    ...
    paths.append(v.EVIDENCE/'candidate03/guests/app-hang/guest.log')
...
def image():
    ...
    return path
def package():
    import check_x86_64_application_udp_media as consumer
    ...
    run('reference-build',[f['tools']['pwsh']['path'],'-NoProfile','-File','scripts/build-x86_64-bootstrap.ps1',
        '-NativeAppNetwork','-OutputDirectory',folder.relative_to(v.ROOT).as_posix()],300)
    run('reference-media',[sys.executable,'scripts/build_x86_64_application_udp_media.py','--input-directory',
    ...
    v.save(v.BASE/'package.json',dict(passed=True,image=v.link(folder/'x86_64/reist-x86_64-bootstrap.elf'),
        index=v.link(media/'application-udp-media.json'),artifacts={p.relative_to(v.ROOT).as_posix():v.digest(p)
        for p in folder.rglob('*') if p.is_file() and 'cache' not in str(p) and p.suffix in ('.elf','.prg','.bin','.map','.o')}))
def runtime():
    import run_qemu_x86_64_application_udp as guest
    ...
    for name in guest.CASES:
        ...
        v.save(v.BASE/(name+'.json'),row);print('APPLICATION_UDP_CASE_OK',name,flush=True)
    v.need(sum(row['elapsed'] for row in rows)<=3600,'total twenty-guest runtime bound')
    v.save(v.BASE/'matrix.json',dict(passed=True,cases=rows))
def review():
    import run_qemu_x86_64_application_udp as guest
    v=evidence;f=binding();img=image();matrix=v.read(v.BASE/'matrix.json')
    v.need(matrix['passed'] and len(matrix['cases'])==20,'twenty fresh cases')
    ...
    print('APPLICATION_UDP_REVIEW_OK')
def gates():

"""Independent signed DNS profile consumer; accepted BL profile stays strict."""
...
NAMES=('boot.prg','cat.prg','ls.prg','probe.prg','data.txt','netdrv.prg','netstack.prg','udp.prg','nc.prg','nslookup.prg')
selected=clone(legacy,[
 ("PROFILE='research-native-shell-two-media-v1'","PROFILE='research-native-application-dns-two-media-v1'"),
 ("'data':'primary-master-read-only-ext2-1k'","'data':'primary-master-read-only-application-dns-ext2-1k'"),
 ("'file-program.prg':1280","'file-program.prg':274432, 'cat.prg':274432, 'ls.prg':274432, 'probe.prg':274432, 'data.txt':16384, 'netdrv.prg':274432, 'netstack.prg':274432, 'udp.prg':274432, 'nc.prg':274432, 'nslookup.prg':274432"),
 ("'system.ext2':131072","'system.ext2':1048576"),('64<=len(app)<=1280','1536<len(app)<=274432'),
 ("directory/'shell-media.json'","directory/'application-dns-media.json'"),
 ("'file-program.prg',*(f'program{n}.prg' for n in range(4)))",
  "'file-program.prg',*(f'program{n}.prg' for n in range(4)),'cat.prg','ls.prg','probe.prg','data.txt','netdrv.prg','netstack.prg','udp.prg','nc.prg','nslookup.prg')"),
 ("data_volume(bounded(attempt/'system.ext2',131072),values['file-program.prg'])",
  "data_volume(bounded(attempt/'system.ext2',1048576),values)")],'reist_application_dns_consumer')
...
def input_binding(values):
    need(set(values)==set(accepted.PINS)|{'netdrv.prg','netstack.prg','udp.prg','nc.prg','nslookup.prg'},'exact DNS profile files')
...
source=once(source,'(32,1024,12,1,0,0,1024,1024,32,1,11,2)','(32,1024,11,1,0,0,1024,1024,32,1,11,2)')
source=once(source,"b'\\xff\\xff\\x0f\\x00'","b'\\xff\\xff\\x1f\\x00'")
source=once(source,'(free,free,12,1)','(free,free,11,1)')
source=once(source,"'free_inodes':12","'free_inodes':11")
namespace={**vars(geometry),'NAMES':NAMES}
exec(compile(source,'<application-dns-ext2-consumer>','exec'),namespace)
...
if __name__=='__main__':
    ...
    print('APPLICATION_DNS_MEDIA_VERIFY_OK',verify(a.directory,a.index,a.openssl))

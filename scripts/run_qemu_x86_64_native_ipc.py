...
def observer_commands(s,c,log,bulk=False):
    code=process.observer_commands(s,log)
    native={n:c['symbols'][n] for n in ('clients','pending','ipc_capability_records')}
    if [native[n]['size'] for n in native] != [432,8576,2048]:raise ValueError('native IPC object layout drift')
    client=native['clients']['value'];pending=native['pending']['value'];caps=native['ipc_capability_records']['value']
    # Batch the identical full-byte checks into three read-only remote reads.
    # One debugger roundtrip per byte would consume the unchanged10s lease.
    fence=f'''python
slot=int(gdb.parse_and_eval('$slot'))
generation=int(gdb.parse_and_eval('*(unsigned long long*)($task+8)'))
if any(mem({client}+slot*108,108)) or any(mem({pending}+slot*2144,2144)): gdb.execute('quit 21')
records=mem({caps},2048)
for offset in range(0,2048,32):
    if records[offset] and struct.unpack_from('<2I',records,offset+16)==(generation,generation): gdb.execute('quit 22')
end
'''
    ...
    def stop(self):
        ...
        try:
            ...
            if self.kind in ('BLOCK','READY'):
                data=mem({pending}+slot*2144,2144)
                nr,a0,a1,a2,a3,result,deadline,ready=struct.unpack('<5Qq2Q',data[:64])
                pgen,pstate=struct.unpack('<2I',data[2136:2144])
                ...
                if self.kind=='BLOCK':
                    ...
                    assert data[:2136]==mem(reg('r13'),2136)
                ...
                assert capacity=={2060 if bulk else 140}
                assert mem(destination,capacity)==request[64:64+capacity]
                version,size,length=struct.unpack_from('<3I',request,64)
                if {bulk!r}:
                    assert (destination&4095)+capacity>4096
                    if version==2:
                        assert (size,length)==(2060,2048)
                        tag=request[76]
                        assert all(request[76+i]==(i*17+tag)&255 for i in range(2048))
                    else:
                        assert (version,size,length)==(1,140,4) and not any(request[80:2124])
                else: assert (version,size,length)==(1,140,4)
                gdb.write('NATIVE_IPC_COPYOUT slot=%d gen=%d bytes=%d cr3=1\\n'%(slot,gen,capacity))
...
def validate_trace(trace,rows,case,bulk=False):
    if 'NATIVE_IPC_OBSERVER_FAIL' in trace:raise ValueError('native IPC observer failure')
    process.validate_trace(trace,rows)
    validate_ipc_trace(trace,rows,case,bulk)


def validate_ipc_trace(trace,rows,case,bulk=False):
    ...
    capacity=2060 if bulk else 140
    count=16 if bulk else 28
    copies=re.findall(r'NATIVE_IPC_COPYOUT slot=(\d+) gen=(\d+) bytes='+str(capacity)+r' cr3=1',trace)
    if len(copies)!=count or trace.count('NATIVE_IPC_COPYOUT')!=count:raise ValueError('IPC actual copied-message count')
...
def main(bulk=False):
    ...
    try:
        for case in range(4):
            ...
            if bulk:cmd.append('-NativeBulkIPC')
            ...
            expected=hashes
            serial,trace=capture(image,folder,observer_commands(symbols(image),c,folder/'frame-trace.log',bulk))
            rows=validate(serial,case);validate_trace(trace,rows,case,bulk)

"""Signed six-file native C/C++ medium using retained CLI mechanisms."""
...
source=once(source,'(16,12)','(16,15)')
source=once(source,'(1<<20)-1','(1<<17)-1')
source=once(source,'u16(2048+14,12)','u16(2048+14,15)')
namespace={**vars(geometry),'NAMES':NAMES}
exec(compile(source,'<cpp-runtime-ext2-producer>','exec'),namespace)
image=namespace['image']
selected=check.clone(legacy,[
    ('scripts/check_x86_64_shell_media.py','scripts/check_x86_64_cpp_runtime_media.py'),
    ("output/'shell-media.json'","output/'cpp-runtime-media.json'"),
    ("for name in ('file-program.prg',*(f'program{n}.prg' for n in range(4))):",
     "for name in ('file-program.prg',*(f'program{n}.prg' for n in range(4)),'cat.prg','ls.prg','probe.prg','cpptest.prg'):"),
    ...
    ("'system.ext2 is the separate primary ATA master, read-only, containing /boot.prg.\\n'",
     "'system.ext2 is read-only and contains the ordinary native cpptest consumer.\\n'")],
    'reist_cpp_runtime_producer')

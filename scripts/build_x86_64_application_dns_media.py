"""Signed ten-file immutable native application DNS profile."""
from pathlib import Path
import argparse,inspect,sys
import build_x86_64_shell_media as legacy
import build_x86_64_graphical_media as geometry
import check_x86_64_application_dns_media as check
from run_qemu_x86_64_runtime_clock import once
NAMES=check.NAMES
source=inspect.getsource(geometry.image)
source=once(source,'(16,12)','(16,11)')
source=once(source,'(1<<20)-1','(1<<21)-1')
source=once(source,'u16(2048+14,12)','u16(2048+14,11)')
namespace={**vars(geometry),'NAMES':NAMES}
exec(compile(source,'<application-dns-ext2-producer>','exec'),namespace)
image=namespace['image']
selected=check.clone(legacy,[
 ('scripts/check_x86_64_shell_media.py','scripts/check_x86_64_application_dns_media.py'),
 ("output/'shell-media.json'","output/'application-dns-media.json'"),
 ("for name in ('file-program.prg',*(f'program{n}.prg' for n in range(4))):",
  "for name in ('file-program.prg',*(f'program{n}.prg' for n in range(4)),'cat.prg','ls.prg','probe.prg','netdrv.prg','netstack.prg','udp.prg','nc.prg','nslookup.prg'):"),
 ...
 'reist_application_dns_producer')

...
class BuildDependencyTests(unittest.TestCase):
    def test_windows_build_resolves_standard_msys2_and_winget_tools(self) -> None:
        ...
        self.assertIn("Resolve-NativeTool 'perl'", script)
        self.assertIn("Resolve-NativeTool 'gperf'", script)
        self.assertIn("(Join-Path $MsysBin 'perl.exe')", script)
        self.assertIn("(Join-Path $MsysBin 'gperf.exe')", script)

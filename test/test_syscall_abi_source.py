...
class SyscallAbiSourceTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.process = read("kernel/proc/process.h")

    def test_common_list_is_complete_append_only_v1(self) -> None:
        self.assertIn("REIST_SYSCALL_ABI_VERSION 1U", self.common)
        self.assertIn("REIST_SYSCALL_COUNT 133U", self.common)
        entries = re.findall(
            r"^\s*X\(([A-Z][A-Z0-9_]*), ([A-Z][A-Z0-9_]*), "
            r"([0-9]+)U\)", self.common, re.MULTILINE)
        self.assertEqual(len(entries), 133)
        self.assertEqual([int(item[2]) for item in entries], list(range(133)))
        self.assertEqual(len({item[0] for item in entries}), 133)
        self.assertEqual(len({item[1] for item in entries}), 133)
        ...
        self.assertEqual(entries[121], ("LSEEK", "LSEEK", "121"))
        self.assertEqual(entries[122], ("FSTAT", "FSTAT", "122"))
        self.assertEqual(entries[123],
                         ("FTRUNCATE", "FTRUNCATE", "123"))
        self.assertEqual(entries[124],
                         ("STORAGE_BULK", "STORAGE_BULK", "124"))
        self.assertEqual(entries[125],
                         ("KERNEL_LOG_READ", "KERNEL_LOG_READ", "125"))
        self.assertEqual(entries[126],
                         ("CPU_TOPOLOGY", "CPU_TOPOLOGY", "126"))
        self.assertEqual(entries[127], ("TERMINAL_INPUT", "TERMINAL_INPUT", "127"))
        self.assertEqual(entries[128], ("PROCESS_RESTRICT", "PROCESS_RESTRICT", "128"))
        self.assertEqual(entries[129], ("FILE_OBJECT_GUARD", "FILE_OBJECT_GUARD", "129"))
        self.assertEqual(entries[130], ("STORAGE_JOURNAL_IO", "STORAGE_JOURNAL_IO", "130"))
        self.assertEqual(entries[131], ("TERMINAL_WRITE_COLOR", "TERMINAL_WRITE_COLOR", "131"))
        self.assertEqual(entries[132], ("TASK_CONTROL", "TASK_CONTROL", "132"))
        self.assertIn("PROCESS_DOMAIN_SYSCALL_LIMIT 133U", self.process)
        self.assertIn("PROCESS_DOMAIN_SYSCALL_WORDS 5U", self.process)
    ...
    def test_check_mode_is_read_only_and_in_sync(self) -> None:
        ...
        self.assertIn("SYSCALL_ABI_CHECK_OK count=133", result.stdout)
    ...
    def test_common_errno_subset_uses_standard_values(self) -> None:
        expected = {
            ...
            "EINVAL": 22, "EMFILE": 24, "ENOSPC": 28, "ESPIPE": 29,
            "EROFS": 30,

...
class TerminalInputTests(unittest.TestCase):
    ...
    def test_all_terminal_aliases_share_admission_and_fixed_wait_node(self):
        ...
        process = (ROOT / "kernel/proc/process.c").read_text()
        legacy = function_block(process, "int process_terminate(")
        identity = function_block(process, "int process_terminate_identity(")
        self.assertIn("return process_terminate_generation(pid, 0U);", legacy)
        self.assertIn("if (generation == 0U) return -1;", identity)
        self.assertIn("return process_terminate_generation(pid, generation);", identity)
        terminate = function_block(process, "static int process_terminate_generation(")
        self.assertIn("process_list[i].generation == generation", terminate)

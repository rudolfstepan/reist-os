...
typedef struct {
    ...
    reist_fs_server_v2 fs;
    reist_block_server block;
    reist_block_profile_v2 profile;
...
static int read_sector(void *p,uint32_t lba,unsigned char *data,uint64_t deadline){
    ...
    if(lba>=h->sectors || h->now>=deadline ||
       h->now-h->previous_read<(h->reads?50:100))return -5;
...
static int block_send(void *p,const x86os_ipc_message_t *q,unsigned ms){
    ...
    (void)reist_block_dispatch_profile_v2(&h->block,&h->profile,&b,q,&h->block_reply);return 0;
}
...
static int fs_send(void *p,const x86os_ipc_bulk_message_t *q,unsigned ms){
    ...
    (void)reist_fs_dispatch_v2(&h->fs,q,&h->fs_reply);return 0;
}
static int fs_receive(void *p,x86os_ipc_bulk_message_t *q,unsigned ms){
    ...
    if(h->receives==h->at){
        ...
        if(h->fault==4)h->now+=REIST_WIDE_FILE_MS;
...
int main(int argc,char **argv){
    ...
    CHECK(!reist_x64_image_prepare_v2(expected,input,bytes));
    ...
    h.profile=(reist_block_profile_v2){2,24,REIST_WIDE_BLOCK_REQUESTS,0,h.now+REIST_WIDE_FILE_MS};
    ...
    int rejected_result=reist_block_dispatch_profile_v2(&h.block,&h.profile,&pacing_backend,&pacing_request,&h.block_reply);
    ...
    memcpy(pacing_request.payload,&pacing_header,sizeof(pacing_header));
    int pacing_result=reist_block_dispatch_profile_v2(&h.block,&h.profile,&pacing_backend,&pacing_request,&h.block_reply);
    ...
    memcpy(pacing_request.payload,&pacing_header,sizeof(pacing_header));
    CHECK(!reist_block_dispatch_profile_v2(&h.block,&h.profile,&pacing_backend,&pacing_request,&h.block_reply));
    CHECK(h.now==250 && h.reads==2 && h.block.requests==3);
    }
    h.now=h.previous_read=100;h.reads=0;h.fault=0;
    CHECK(!reist_block_server_init(&h.block,pacing_owner,h.sectors,h.now));
    ...
    for(trial=0;trial<(unsupported?1U:20U);trial++){
        ...
        h.profile=(reist_block_profile_v2){2,24,REIST_WIDE_BLOCK_REQUESTS,0,h.now+REIST_WIDE_FILE_MS};
        CHECK(!reist_block_profile_admit_v2(&h.profile,h.now));
        ...
        reist_fs_profile_v2 fp={2,40,argv[2][0]=='f'?REIST_FS_FAT:REIST_FS_EXT2,h.sectors,owner,driver,h.profile.deadline_ms};
        CHECK(!reist_fs_server_init_v2(&h.fs,&fp,&bt));
        reist_fs_client client={0};CHECK(!reist_fs_client_bind(&client,owner));
        reist_fs_transport ft={&h,clock_ms,fs_send,fs_receive};
        reist_file_capture_v2 observation;
        const char *path=argv[2][0]=='f'?"/BOOT.PRG":"/boot.prg";
        CHECK(!reist_x64_file_stat_v2(&observation,&client,&ft,path,9,h.profile.deadline_ms));
        CHECK(observation.version==2 && observation.deadline_ms==h.profile.deadline_ms);
        unsigned before=h.calls;int error=unsupported?-110:0,untouched=0;
        ...
        if(trial==4){observation.frame.stat.info.size=REIST_WIDE_FILE_BYTES+1;error=-27;untouched=1;}
        ...
        if(trial==15){client.sequence=observation.sequence=REIST_WIDE_FS_REQUESTS-1;error=-11;untouched=1;}
        ...
        int result=reist_x64_file_finish_v3(output,&work,&client,&ft,&observation);
        ...
        CHECK(h.calls<=REIST_WIDE_FS_REQUESTS && h.reads<=REIST_WIDE_BLOCK_REQUESTS);
        if(untouched)CHECK(h.calls==before);
        if(!result){
            CHECK(h.calls==(bytes+255)/256+2);
            CHECK(reist_x64_file_finish_v3(output,&work,&client,&ft,&observation)==-116);
        }
        CHECK(!reist_fs_server_fence_v2(&h.fs));
        CHECK(h.fs.state.failed && !h.fs.state.used && !h.fs.next_slot);
        CHECK(uniform(h.fs.state.data,sizeof(h.fs.state.data),0));
    }
    free(h.media);puts("WIDE_FILE_PASS");return 0;
}

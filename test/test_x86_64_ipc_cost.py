...
import hashlib,itertools,os,re,subprocess,sys,unittest,uuid,json
...
class IPCCostTests(unittest.TestCase):
    def test_actual_counts_semantics_and_corruption(self):
        ...
        folder=ROOT/'build/codex-agent/r83bc-application-files'/('ipc-cost-'+uuid.uuid4().hex);folder.mkdir(parents=True)
        ...
        pool_reference=folder/'reference-pool.c'
        pool_reference.write_bytes(subprocess.check_output(
            ['git','show','2e05a01d:arch/x86_64/ipc/native_ipc.c'],cwd=ROOT,timeout=10))
        current=ROOT/'arch/x86_64/ipc/native_ipc.c'
        (folder/'sources.json').write_text(json.dumps({name:hashlib.sha256(path.read_bytes()).hexdigest() for name,path in [('reference',old),('reference-pool',pool_reference),('candidate',current)]},indent=2))
        ...
        for opt,pool in itertools.product(('0','2'),('0','1')):
            transcripts={}
            for name,path in [('reference',pool_reference if pool=='1' else old),('candidate',current)]:
                tag=name+opt+'-pool'+pool
                exe=folder/(tag+'.exe')
                run([find_zig(),'cc','-std=c11','-Wall','-Wextra','-Werror','-O'+opt,
                     ...
                     '-DREIST_NATIVE_TASK_POOL='+pool,
                     '-DIPC_SOURCE="'+path.as_posix()+'"','-I.','-Iarch/x86_64/ipc',
                     'kernel/ipc/ipc.c','kernel/init/critical_object.c','test/x86_64_ipc_cost_host.c','-o',exe],tag+'-build')
                transcripts[name]=run([exe],tag+'-run')
                self.assertIn('IPC_COST_HOST_OK',transcripts[name])
                for slot in range(8 if pool=='1' else 4):
                    self.assertIn('IPC_COST_FAULT_OK',run([exe,str(slot)],tag+'-fault-'+str(slot)))
            ...
            for old_row,new_row in zip(old_rows,new_rows):
                ...
                expected={'idle':0,'wait-v1':0,'wait-v2':0,'retry-v1':0,'retry-v2':0,
                          'take':5,'empty-take':0,'bind':2,'end':0}
                if label in expected:self.assertEqual(updates,expected[label],label)
                self.assertLessEqual(updates,int(old_row[2]),label)
            print('IPC_COST_EQUIVALENT O'+opt+' pool='+pool+' '+str(folder),flush=True)

...
Engine oder DOM in diesem Paket. 28 Hostfaelle, beide Referenzen, beide
Math-Gaeste (1/4 CPUs), libc und Browser-Eingabe/Crash/Restart bestehen.
Kernel beider Profile und vier vorhandene Programme sind SHA256-bytegleich
zu0301d708. Falscher Byte-Linkweg und fehlende Rezeptabhaengigkeit des neuen
Testprogramms sind mit erhaltenen Vorherbelegen repariert. Queue geht nur
formal auf R3.6b weiter; kein Vorziehen vor dokumentiertem Browservorrang.
Details und Grenzen: CURRENT_WORK.md.
...
**R8.2d ist abgeschlossen:** Der erste echte Boot-Orchestrierungsschritt ist
...
Adapter ist lease-geschuetzt, erhaelt die SysV-ABI und bietet keine weitere
Autoritaet. 44 Quellvertragstests, der isolierte Build und der begrenzte
Ein-vCPU-QEMU-Dialog bis `C_KERNEL_CONTROL_OK` und `RING3_SHELL_OK` bestanden;
...
**R8.2j ist abgeschlossen:** Der vollstaendig reaptierte Shell-Kindslot wird
exakt einmal als Generation 42 wiederverwendet. 50 Quellvertragstests, der
warnungsfreie 126.868-Byte-Build und ein begrenzter `INFO`-/`RUN`-/`RUN`-/
`EXIT`-QEMU-Dialog bewiesen beide blockierenden WAIT-/Status-77-Zyklen,
frische private Kindressourcen und die vollstaendige Ruecknahme jeder
...
**R8.2k ist abgeschlossen:** Der feste Pfad `/shell/child` ist an ein separat
gelinktes, 360-Byte-RX-only-System-V-AMD64-`ET_EXEC` gebunden. Genau drei feste
Loaderkontexte trennen Probe, Shell und Kind. 51 Quellvertragstests, der
warnungsfreie 127.488-Byte-Build und der native QEMU-Lauf bewiesen zwei frische
Kindladungen, vollständige Task- und Abbildfreigabe vor jedem Shell-Wakeup und
...
**R8.2m ist abgeschlossen:** Eine parallele feste Vier-Slot-Profiltabelle
bindet je Prozessgeneration genau eine 64-Bit-Syscallmaske, ohne den
256-Byte-Taskrecord oder die ABI zu vergroessern. Generation 40 erhaelt nur die
acht Shelloperationen, Generation 41/42 ausschliesslich `EXIT`. 53
Quellvertragstests, der warnungsfreie 129.680-Byte-Build und der native
QEMU-Lauf bewiesen fuer beide Kindgenerationen `GETPID -> EACCES`, Fortsetzung
ueber IRETQ/Runqueue, anschliessende Stackpruefung und Exit 77. Reap loeschte
jedes Profil generationengenau; vor Erfolg waren alle vier Profile sowie Task-,
Queue-, Relations-, Loader- und Frameautoritaet bereinigt. Dynamische Profile,
...
**R8.2p ist abgeschlossen:** Der Ein-Slot-IPC-Pfad beweist asynchrone
...
`token77` ueber den vorhandenen Timeout-Wakeup-Pfad. 54 Quellvertragstests,
der warnungsfreie 142.800-Byte-Build und zwei reale QEMU-`RUN`-Zyklen bestanden
bis `RING3_SHELL_OK`. Blocking-Sender, tiefere Queues, mehrere Waiter,
Endpointregistries und produktive Integration bleiben Folgearbeiten; die
x86_64-Queue ist leer.

**R8.2q ist abgeschlossen:** `IPC_SEND_TIMEOUT` 53 ergaenzt genau einen festen,
...
Folgearbeiten. 54 Quellvertragstests, der warnungsfreie 145.572-Byte-Build und
...
R7.1d ist abgeschlossen und beseitigt den quadratischen Leserest in Operation 7:
...
Eintraege und eine absolute Fuenf-Sekunden-Frist begrenzt. Die Hostnachweise
halten 128 FAT-Eintraege plus EOF unter 129 Verzeichnis-Datensektorreads und
...
R7.1e ist abgeschlossen. Der erste reale SSD-Lauf schrieb alle 256 KiB und
beendete `fsync`, scheiterte danach aber in der gemeinsamen
Uhr-/90-Sekunden-Auswertung mit `step=write-clock`. Die Bootbilder belegten
AHCI, `hdd0p2` und null PIO-Laufwerke. Die Hauptkorrektur laesst daher
AHCI-Verifikation und die vier Journalbarrieren unveraendert, buendelt jedoch
den validierten `SYS_WRITE`-Benutzerinhalt ueber eine statische,
mutexgeschuetzte 4096-Byte-Stagingseite: Ein Benchmarkchunk ist nun eine statt
acht VFS-/Journaltransaktionen, insgesamt 64 statt 512. Der FAT32-Hostnachweis
schreibt eine ganze Seite innerhalb der festen 20 Undo-Slots, zaehlt weiter
exakt vier Barrieren und verifiziert den Readback bytegleich; der 21.
eindeutige Sektor wird vor der Zielpublikation abgewiesen und der Abbruch
laesst alle Zielsektoren unveraendert. Die
Benchmark-Auswertung trennt Uhrfehler von ihrer eigenen 300-Sekunden-Grenze,
ohne einen I/O-Wait zu verlaengern. Der PIO-Rueckfallpfad waehlt ausserdem
`FLUSH CACHE` und `FLUSH CACHE EXT` aus den getrennten, gueltigen
IDENTIFY-Kommandobits statt pauschal aus LBA48; Fehler bleiben sichtbar und
verriegeln den bestehenden Storage-Write-Pfad. Alle 54 gezielten Pruefungen
und der 17-sekuendige `real_hw/vga`-Paketbuild bestanden. Der manuelle
...
7. [x] **N3a · Begrenzte POSIX-Symlinks auf EXT2 bereitstellen.** Die
   ...
   nicht erweitert. Relative und absolute Ziele, Ketten, Dangling Links und
   Zyklen werden mit festen Pfad-, Tiefen-, I/O- und Zeitbudgets behandelt.
...
11. [ ] **N3 · Verbleibende Dateisystemmutationen über den Ring-3-Dienst
   ...
   `active_id` leer. Der abgeschlossene Schnitt `N3d` ergänzt ausschließlich Unlink
   ...
   vorhandenen 24-Sektor-Undo-Journals scheitern vor Medienwirkung. Abgenommen
   mit 38 finalen Targeted-Checks, einem 68-Sekunden-QEMU-Framebuffer-
   Paketbuild und einem realen 104-Sekunden-EXT2-Lauf mit persistentem DEL,
   ...
   bleiben offen. Der abgeschlossene begrenzte Schnitt
   `N3f-ext2-cross-sector-rename` erlaubt einen fehlenden Zielnamen in genau
   ...
   Cross-Block, Replace und Verzeichnisse bleiben ausgeschlossen. Abgenommen
   mit 38 Targeted-Checks, dem 46-Sekunden-QEMU-Framebuffer-Paketbuild und
   einem finalen 95-Sekunden-EXT2-Lauf, der beide Zielrecords in Sektor 2,
   Dienstneustart, bytegleiche Inode-/Dateidaten und saubere redundante
   Journalheader nachwies. Bis zur Abgrenzung des nächsten N3-Fehlermodells
   bleibt `active_id` leer.
...
inert, CSS, Formulare, Bilder und allgemeine HTML5-Kompatibilität fehlten in
diesem ersten Schnitt. Eine
...
damals war `active_id` leer. Das auf Nutzerwunsch aktive Folgepaket
`R3.6c-browser-interaction-and-images` ergänzt getrennte URL-Zeichnung,

...
#define DEVICE_DOMAIN_GR_EXECUTION_TIMEOUT_MS 5000U
...
enum {
    ...
    DEVICE_DOMAIN_CONTROL_NATIVE_PIO = 29U,
};
...
enum {
    ...
    DEVICE_DOMAIN_DRIVER_REPORT_STARTUP_PROGRESS = 5U,
};
...
typedef struct {
    ...
    uint32_t flags;
    uint32_t reserved[3];
} device_domain_gr_execution_request_t;

enum {
    DEVICE_DOMAIN_GR_EXECUTION_READY = 1U << 0U,
};
...
typedef device_domain_gr_execution_request_t
    device_domain_gr_golden_context_request_t;

enum {
    DEVICE_DOMAIN_GR_GOLDEN_CONTEXT_READY = 1U << 0U,
    DEVICE_DOMAIN_GR_GOLDEN_CONTEXT_OPAQUE_VRAM = 1U << 1U,
};
...
_Static_assert(sizeof(device_domain_gr_execution_request_t) == 40U,
...
/** Execute one validated GK208 LTC/GR pre-start transaction. */
...
/** Generate and retain one opaque topology-bound GK208 golden context. */
